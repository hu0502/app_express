var UserSQL = {
    ...
    enroll:'INSERT INTO user VALUES(null,?,md5(?),?,?,now(),1000,null,null,null,null)',//用户注册
    update:'UPDATE user set password=md5(?),department=?,u_class=?,sex=? WHERE user_id = ?',//更新个人信息
    //用户查询自己已发布任务信息
    getTask:'SELECT * FROM mission WHERE master = ?',
    //用户查询自已已接收的所有任务
    getAccept:'SELECT * FROM mission WHERE slave = ?'
};
